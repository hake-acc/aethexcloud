...
export function ContactPage() {
  return (
    <>
      <section className="pt-32 pb-16 px-6"><div className="mx-auto max-w-[1280px]"><ScrollReveal><div className="flex flex-col items-center text-center gap-4"><SectionLabel>Contact</SectionLabel><h1 className="text-5xl sm:text-6xl font-bold tracking-[-0.04em] text-white">Talk to AethexCloud hosting support</h1><p className="max-w-xl text-lg text-[#A1A1AA]">Have a question about VPS hosting in India, website hosting, Minecraft servers or Discord bots? We are here to help.</p></div></ScrollReveal></div></section>
      ...
      <section className="px-6 pb-24"><div className="mx-auto max-w-[1280px]"><div className="grid grid-cols-1 lg:grid-cols-5 gap-12"><ScrollReveal direction="left" className="lg:col-span-2"><div className="flex flex-col gap-5"><h2 className="text-3xl font-semibold tracking-tight text-white">Contact cloud hosting support</h2><p className="text-[#A1A1AA]">Fill out the form and we will get back to you as soon as possible. You can also review our <a href="/knowledge-base" className="text-white underline underline-offset-4 hover:text-white/70">hosting guides</a> before reaching out.</p><div className="flex flex-col gap-3 mt-2">{["Service questions", "Plan upgrades", "Technical support", "General enquiries"].map((item) => <div key={item} className="flex items-center gap-2.5 text-sm text-[#A1A1AA]"><div className="w-1 h-1 rounded-full bg-white/30 flex-shrink-0" />{item}</div>)}</div></div></ScrollReveal><ScrollReveal direction="right" delay={0.1} className="lg:col-span-3"><ContactForm /></ScrollReveal></div></div></section>
    </>
  );
}